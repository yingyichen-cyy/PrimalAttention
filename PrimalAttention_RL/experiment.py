...
def experiment(
    ...
    if model_type == 'dt':
        model = DecisionTransformer(
            ...
            low_rank=variant['low_rank'],
            rank_multi=variant['rank_multi'],
        )
    elif model_type == 'bc':
        model = MLPBCModel(
            state_dim=state_dim,
...
if __name__ == '__main__':
    ...
    # hyper-parameters for the KSVD block
    parser.add_argument('--low_rank', type=int, default=30)
    parser.add_argument('--rank_multi', type=int, default=10)
    ...
    utils.make_dir(base_dir)
    args.work_dir = os.path.join(base_dir, args.env + '_' + args.dataset + '_rank{}'.format(args.low_rank) + '_eta{}'.format(args.eta))
