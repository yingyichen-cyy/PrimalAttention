...
# for the KSVD parameters
# Accumulate trainable parameters in 2 groups:
# 1. optimize on U matrix; 
# 2. optimize on Network param.
def param_state(model):
    param_we, param_wr, param_else = [], [], []
    for name, param in model.named_parameters():
        if param.requires_grad:
            if 'inner_attention.we' in name:
                param_we.append(param)
            elif 'inner_attention.wr' in name:
                param_wr.append(param)
            else:
                param_else.append(param)
                
    return param_we, param_wr, param_else
