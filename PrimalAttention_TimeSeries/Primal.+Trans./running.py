...
class BaseRunner(object):
    ...
    def __init__(self, model, eta, dataloader, device, loss_module, optimizer=None, l2_reg=None, print_interval=10,
                 console=True):
        self.model = model
        self.eta = eta
...
class UnsupervisedRunner(BaseRunner):
    ...
    def evaluate(self, epoch_num=None, keep_all=True):
        ...
        for i, batch in enumerate(self.dataloader):
            ...
            # Cascade noise masks (batch_size, padded_length, feat_dim) and padding masks (batch_size, padded_length)
            target_masks = target_masks * padding_masks.unsqueeze(-1)
            loss, loss_ce, loss_ksvd = self.loss_module(predictions, targets,
                                    target_masks)  # (num_active,) individual loss (square error per element) for each active value in batch
            batch_loss = torch.sum(loss).cpu().item()
            mean_loss = batch_loss / len(loss)  # mean loss (over active elements) used for optimization the batch

            batch_loss_ce = torch.sum(loss_ce).cpu().item()
            mean_loss_ce = batch_loss_ce / len(loss_ce)

            batch_loss_ksvd = torch.sum(loss_ksvd).cpu().item()
            mean_loss_ksvd = batch_loss_ksvd / len(loss_ksvd)
            ...
            metrics = {"loss_total": mean_loss, "loss_ce": mean_loss_ce, "loss_ksvd": mean_loss_ksvd}
            if i % self.print_interval == 0:
                ending = "" if epoch_num is None else 'Epoch {} '.format(epoch_num)
...
class SupervisedRunner(BaseRunner):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if isinstance(args[-1], torch.nn.CrossEntropyLoss):
    ...
    def train_epoch(self, epoch_num=None):
        ...
        for i, batch in enumerate(self.dataloader):
            ...
            predictions, score_list, Lambda_list = self.model(X.to(self.device), padding_masks)

            loss, loss_ce, loss_ksvd = self.loss_module(Lambda_list, predictions, targets, self.eta, score_list)  # (batch_size,) loss for each sample in the batch
            batch_loss = torch.sum(loss)
            mean_loss = batch_loss / len(loss)  # mean loss (over samples) used for optimization

            batch_loss_ce = torch.sum(loss_ce)
            mean_loss_ce = batch_loss_ce / len(loss_ce)

            batch_loss_ksvd = torch.sum(loss_ksvd)
            mean_loss_ksvd = batch_loss_ksvd / len(loss_ksvd)
    ...
    def evaluate(self, epoch_num=None, keep_all=True):
        ...
        for i, batch in enumerate(self.dataloader):
            ...
            loss, _, _ = self.loss_module(Lambda_list, predictions, targets, self.eta, score_list)
